...
public class U4Activity extends AppCompatActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.w("test",U4Info.toString());


        // レイアウトからリストビューを取得
        ListView listView = (ListView)findViewById(R.id.U4listView);

        // リストビューに表示する要素を設定
        ArrayList<SampleListItem> listItems = new ArrayList<>();
        for (int i = 0; i < U4name.length; i++) {
            Bitmap bmp = BitmapFactory.decodeResource(getResources(), R.drawable.img_u4);
            if (U4Info[i] == 0) {
               bmp = null;
            }
            SampleListItem item = new SampleListItem(bmp, U4name[i]);
            listItems.add(item);
        }
