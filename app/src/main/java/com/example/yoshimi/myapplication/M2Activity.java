...
//        //ここから
//        String[] M2name = {
//                "大黒 智貴","嶋川 司","高谷 友貴","田村 聡明",
//                "田村 聡明","富岡 亮登","冨田 龍太郎","那須 大晃",
//                "本田 雄亮","山下 俊樹", "山本 泰士","Antoine Janssens"
//        };

        // レイアウトからリストビューを取得
        ListView listView = (ListView)findViewById(R.id.M2listView);

        // リストビューに表示する要素を設定
        ArrayList<SampleListItem> listItems = new ArrayList<>();
        for (int i = 0; i < M2name.length; i++) {
            Bitmap bmp = BitmapFactory.decodeResource(getResources(), R.drawable.img_m2);
            if (M2Info[i] == 0) {
                bmp = null;
            }
            SampleListItem item = new SampleListItem(bmp, M2name[i] );
            listItems.add(item);
        }
